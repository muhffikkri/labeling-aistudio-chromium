...
class ExecutionMetricsTracker:
    ...
    def _save_to_csv(self):
        ...
        try:
            ...
            # Write to CSV with atomic append (create temp file first)
            temp_file = self.csv_file.with_suffix('.csv.tmp')
            
            # If file exists, copy existing content first
            if file_exists:
                import shutil
                shutil.copy2(self.csv_file, temp_file)
                
            # Append new data to temp file
            with open(temp_file, 'a' if file_exists else 'w', newline='', encoding='utf-8', errors='replace') as f:
